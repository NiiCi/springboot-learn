...
public class test {
    ...
    public void optionalTest() {
        ...
        Assert.assertEquals("c", emptyOptional.orElseGet(() -> {
            return "c";
        }));
    }
    ...
    public void partitioningByTest() {
        ...
        Map<Boolean, List<User>> newList = userList.stream().collect(Collectors.partitioningBy(user -> {
            return ("user2").equals(user.getName());
        }));
    ...
    /**
     * 策略模式
     * 能在运行时改变软件的算法行为,例如 压缩文件,我们提供了各种压缩的方式,可以使用 zip gzip 算法
     */
    @Test
    public void testCL(){
        Path inFile = null;
        ...
        try {
            gzipCompressor.compressor(inFile,outFile);
        } catch (IOException e) {
    ...
    /**
     * 观察者模式
     * 以 NASA 和 外星人 观察 Moon 为例
     * 当 human 或者 lambda 着陆时,会触发相应的操作
     * 定义了一个观察者接口 LandingObserver,提供了 observeLanding 方法 用于观察
     * Nasa 和 Aliens 实现了 landingObserver 接口,实现 observerLanding 方法
     * Moon 为被观察者,持有一个 观察者列表 , 一个着陆的 land 方法和一个添加观察者的 startSpying 方法
     */
    @Test
    public void testGCZ(){
       Moon moon = new Moon();
       ...
       //使用 lambda
        moon.startSpying(name ->{
            if (name.contains("lambda")){
                log.info("lambda ------ 我着陆到月球了!!!");
            }
        });
        moon.startSpying(name->{
            if (name.contains("lambda")){
                log.info("检测到 lambda 着陆,可以开始入侵地球!!!");
            }
        });
        moon.land("human");
        moon.land("lambda");
